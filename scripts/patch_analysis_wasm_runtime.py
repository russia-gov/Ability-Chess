...
new_request = r'''  const ANALYSIS_ABILITYFISH_WORKER='./analysis-engine/abilityfish-worker.js';
  const ANALYSIS_ABILITY_NAMES=['Move','Shield','Freeze','Bomb','Swap','Swap','Recall','Ambush','Teleport','Reinforce','Portal','Portal','Fortify','Double Move','Upgrade'];
  ...
  function analysisAbilityLabel(entry){
    ...
    const name=ANALYSIS_ABILITY_NAMES[a.kind]||`Ability ${a.kind}`;
  ...
  function convertWasmAnalysisResult(state,result){
    ...
    return {score:primaryScore,depth:Number(primary?.depth||result?.depth||1),nodes:Number(primary?.nodes||0),displayLines,wasm:true};
  }
  function requestAbilityFishAnalysis(state,options={}){
    return new Promise((resolve,reject)=>{
      ...
      const id=`analysis-${++analysisWorkerRequestSeq}-${Date.now()}`;let settled=false;
      const cleanup=()=>{clearTimeout(timeout);analysisWorkers.delete(worker);try{worker.terminate();}catch{}};
      const timeout=setTimeout(()=>{if(settled)return;settled=true;cleanup();reject(new Error('AbilityFish depth search timed out'));},120000);
      const onMessage=e=>{
      ...
      worker.postMessage({id,fen:analysisStateFen(state),depth,multiPV:3,abilityState:analysisAbilityPayload(state)});
    });
  }
'''

# The file may use CRLF; normalize only for matching and restore original style later.
newline = '\r\n' if '\r\n' in s else '\n'
...
# The old quick pass deliberately disabled abilities. The WASM runtime is fast enough
# at depth 1 and must always receive the complete Ability Chess state.
normalized = normalized.replace(
