...
int main(){
  ...
  frozen.side[0].lastMove={Square{28},Square{12},3,true};
  acts=generate_meta_actions(frozen,b);
  for(auto a:acts) {
    assert(!(a.kind==ActionKind::Teleport && a.from==12));
    assert(!(a.kind==ActionKind::Recall && a.from==12));
    assert(!(a.kind==ActionKind::SwapFinish && (a.from==12 || a.to==12)));
  }
  r=apply_action(frozen,b,{ActionKind::Teleport,12,28,0,0}); assert(!r.ok);
  r=apply_action(frozen,b,{ActionKind::Recall,12,28,0,0}); assert(!r.ok);
  r=apply_action(frozen,b,{ActionKind::SwapFinish,12,20,0,0}); assert(!r.ok);
